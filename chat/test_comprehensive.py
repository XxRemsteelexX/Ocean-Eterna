...
# /query/code-discussions - expects chunk_id
# Use a real chunk ID from the corpus
r = requests.post(f"{BASE}/query/code-discussions",
    json={"chunk_id": "guten9m.1"}, timeout=30)
test("/query/code-discussions returns 200", r.status_code == 200)
data = r.json()
test("/query/code-discussions has chunk_id field", "chunk_id" in data)

# /query/fixes - expects filename
r = requests.post(f"{BASE}/query/fixes",
    json={"filename": "test.cpp"}, timeout=30)
test("/query/fixes returns 200", r.status_code == 200)
data = r.json()
test("/query/fixes has filename field", "filename" in data)

# /query/feature - expects feature_id
r = requests.post(f"{BASE}/query/feature",
    json={"feature_id": "feature1"}, timeout=30)
test("/query/feature returns 200", r.status_code == 200)
data = r.json()
test("/query/feature has feature_id field", "feature_id" in data)
