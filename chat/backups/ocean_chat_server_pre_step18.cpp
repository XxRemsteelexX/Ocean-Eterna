...
// Compress chunk with LZ4 (kept for reference/fallback)
string compress_chunk_lz4(const string& input) {
...
json handle_chat(const string& question) {
    ...
    // Use hit_metas extracted earlier (already thread-safe)
    for (const auto& meta : hit_metas) {
        ChunkReference ref;
        ref.chunk_id = meta.id;
        ref.relevance_score = meta.score;

        // Get snippet from the decompressed context
        string chunk_content = decompress_chunk(global_storage_path, meta.offset, meta.length);
