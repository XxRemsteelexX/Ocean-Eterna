...
// Only visits docs containing query terms instead of brute-force 5M scan
vector<Hit> search_bm25(const Corpus& corpus, const string& query, int topk) {
    ...
    unordered_map<uint32_t, double> doc_scores;

    // TAAT: for each stemmed query term, traverse posting lists
    for (const string& term : query_terms) {
        auto stk_it = g_stem_to_keywords.find(term);
        if (stk_it == g_stem_to_keywords.end()) continue;

        // Compute df for this stemmed term (sum across all original keywords)
        int df = 0;
        ...
        // Traverse posting lists for all original keywords that stem to this term
        for (const string& orig_kw : stk_it->second) {
            auto inv_it = corpus.inverted_index.find(orig_kw);
            if (inv_it == corpus.inverted_index.end()) continue;

            for (uint32_t doc_idx : inv_it->second) {
                // tf=1 per posting list entry (keywords are unique per doc)
                // If multiple original keywords stem to same term, tf accumulates
                int doc_len = corpus.docs[doc_idx].keywords.size();
                double norm = k1 * (1.0 - b + b * doc_len / corpus.avgdl);
                // Use tf=1 per keyword match; accumulation handles multi-keyword stems
    ...
    // Extract top-K results using partial sort
    vector<pair<uint32_t, double>> scored_docs(doc_scores.begin(), doc_scores.end());
    int result_count = min((int)scored_docs.size(), topk);
    if (result_count > 0) {
        partial_sort(scored_docs.begin(),
                     scored_docs.begin() + result_count,
                     scored_docs.end(),
                     [](const pair<uint32_t, double>& a, const pair<uint32_t, double>& b) {
