...
const string EXTERNAL_API_URL = get_env_or("OCEAN_API_URL", "https://routellm.abacus.ai/v1/chat/completions");
const string EXTERNAL_API_KEY = get_env_or("OCEAN_API_KEY", "");
const string EXTERNAL_MODEL = get_env_or("OCEAN_MODEL", "gpt-5-mini");
...
void run_http_server() {
    ...
    while (true) {
        ...
        if (request.find("GET /stats") != string::npos) {
            ...
            if (body_start != string::npos) {
                string body = request.substr(body_start + 4);
                DEBUG_LOG("add-file-path received body: " << body.substr(0, 200));
                try {
                    json req = json::parse(body);
                    DEBUG_LOG("Parsed JSON: " << req.dump());
...
int main(int argc, char** argv) {
    ...
    // Check for API key
    if (EXTERNAL_API_KEY.empty() && USE_EXTERNAL_API) {
