...
RefinementResult search_with_refinement(const string& question, int topk, int max_iter = 3) {
    ...
    for (int iter = 0; iter <= max_iter; iter++) {
        ...
        {
            shared_lock<shared_mutex> lock(corpus_mutex);
            if (global_shards.shard_count > 0) {
...
json handle_chat(const string& question) {
    ...
    // Use refinement loop for all queries (binary: FOUND/REFINE)
    auto refine_result = search_with_refinement(question, effective_topk);
    ...
    svr.Post("/search", [](const httplib::Request& req, httplib::Response& res) {
        try {
            ...
            if (use_refinement && !g_config.llm.api_key.empty()) {
                ...
                // fallback: raw BM25 + source coherence
                shared_lock<shared_mutex> lock(corpus_mutex);
                if (global_shards.shard_count > 0) {
                    hits = search_unified_twophase(global_shards, global_corpus, query, topk);
                } else {
                    hits = search_bm25_twophase(global_corpus, query, topk);
                }
                apply_source_coherence(hits, global_corpus);
            }
            ...
            vector<Hit> hits;
            {
                ...
                if (is_self_referential_query(question)) {
                    ...
                    hits = global_shards.shard_count > 0
                        ? search_unified_twophase(global_shards, global_corpus, question, effective_topk)
                        : search_bm25_twophase(global_corpus, question, effective_topk);
                    for (auto& hit : hits) {
                        if (hit.doc_idx < global_corpus.docs.size() &&
                            has_prefix(global_corpus.docs[hit.doc_idx].id, "CH")) {
                            hit.score *= 1.5;
