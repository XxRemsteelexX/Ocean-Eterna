...
int main(int argc, char** argv) {
    ...
    cout << "bulk_build v9.0 (two-pass disk-backed): " << threads << " threads, target "
    ...
    for (size_t fi = 0; fi < files.size() && !target_reached; ) {
        ...
        for (size_t bi = 0; bi < batch_count && !target_reached; bi++) {
            for (auto& chunk : batch_results[bi]) {
                ...
                if (running_tokens >= max_tokens) { target_reached = true; break; }
            }
            files_done++;
        }
    ...
    cout << "  generating stems.idx..." << endl;
    {
        ...
        // step 4: first pass — collect unique stems and their DF (no posting lists yet)
        // this is ~2GB at 50B (50M stems * ~40 bytes each for string + hash + df)
        cout << "    collecting unique stems..." << endl;
        struct StemInfo {
            string stem;
            uint64_t hash;
            uint32_t df;    // filled later during bucketed pass
        };
        unordered_map<string, uint32_t> stem_df_map;  // stem -> doc count
        for (size_t i = 0; i < NM; i++) {
            auto kws = read_chunk_keywords(kw_sorted_mmap.data, all_slim[i].kw_file_offset);
            unordered_set<string> chunk_stems;
            for (const auto& [kid, tf] : kws) {
                const string& kw = kw_intern.id_to_str[kid];
                if (kw_df[kid] <= 1 && kw.find('_') != string::npos) continue;
                chunk_stems.insert(kw_stems[kid]);
            }
            for (const string& stem : chunk_stems) {
                stem_df_map[stem]++;
            }
        }

        // sort stems by hash for binary search (directory order)
        vector<StemInfo> all_stems;
        all_stems.reserve(stem_df_map.size());
        for (auto& [stem, df] : stem_df_map) {
            uint64_t h = 0xcbf29ce484222325ULL;
            for (char c : stem) { h ^= (uint8_t)c; h *= 0x100000001b3ULL; }
            all_stems.push_back({stem, h, df});
        }
        stem_df_map.clear();  // free the map
        sort(all_stems.begin(), all_stems.end(),
        ...
        // build stem -> sorted index mapping for keyword_id -> stem_id lookup
        unordered_map<string, uint32_t> stem_to_sorted_idx;
        stem_to_sorted_idx.reserve(sc);
        for (size_t i = 0; i < sc; i++) {
            stem_to_sorted_idx[all_stems[i].stem] = i;
        }

        // step 5: write keywords.idx (needs stem_to_sorted_idx, does it now before we free stems)
        string kw_path = out_dir + "/keywords.idx";
        {
            ...
            for (size_t k = 0; k < NKW; k++) {
                auto it = stem_to_sorted_idx.find(kw_stems[k]);
                uint32_t sid = (it != stem_to_sorted_idx.end()) ? it->second : UINT32_MAX;
                kf.write((char*)&sid, 4);
            }
            kf.close();
        }
        stem_to_sorted_idx.clear();

        // step 6: bucketed posting list construction
        // partition stems into N_BUCKETS by their index in the sorted array.
        // for each bucket, scan all chunks, collect postings only for stems in that bucket.
        // this limits peak memory to ~1/N of total posting data.
        const size_t N_BUCKETS = max((size_t)1, min((size_t)32, (sc + 500000) / 500000));
        cout << "    building posting lists in " << N_BUCKETS << " buckets..." << endl;

        // for each stem, we need to know which bucket it belongs to
        // bucket i covers stems with sorted index in [bucket_start[i], bucket_start[i+1])
        vector<size_t> bucket_starts(N_BUCKETS + 1);
        for (size_t b = 0; b <= N_BUCKETS; b++) {
            bucket_starts[b] = b * sc / N_BUCKETS;
        }

        // build keyword_id -> stem sorted index for fast lookup during scanning
        // (we need to know which bucket each keyword's stem falls in)
        vector<uint32_t> kw_stem_sorted_idx(NKW, UINT32_MAX);
        {
            // rebuild the mapping since we cleared stem_to_sorted_idx
            unordered_map<string, uint32_t> stem_idx_map;
            stem_idx_map.reserve(sc);
            for (size_t i = 0; i < sc; i++) {
                stem_idx_map[all_stems[i].stem] = i;
            }
            #pragma omp parallel for schedule(static)
            for (size_t k = 0; k < NKW; k++) {
                auto it = stem_idx_map.find(kw_stems[k]);
                if (it != stem_idx_map.end()) {
                    kw_stem_sorted_idx[k] = it->second;
                }
            }
        }
        // free kw_stems to save memory (no longer needed after this)
        { vector<string>().swap(kw_stems); }

        // write stems.idx in stages
        // first: write header + directory entries + stem strings (known from all_stems)
        // then: write posting lists bucket by bucket
        ...
        sf.write((char*)&sc, 8);
        uint64_t ps_placeholder = 0; sf.write((char*)&ps_placeholder, 8); // will patch
        ...
        // write posting lists bucket by bucket
        for (size_t b = 0; b < N_BUCKETS; b++) {
            size_t b_start = bucket_starts[b];
            size_t b_end = bucket_starts[b + 1];
            size_t b_size = b_end - b_start;
            if (b_size == 0) continue;

            // for stems in this bucket, collect posting lists
            // key: stem sorted index (relative to b_start), value: posting list
            vector<vector<uint32_t>> bucket_postings(b_size);
            ...
            if (N_BUCKETS > 1) {
                cout << "    bucket " << (b+1) << "/" << N_BUCKETS
                     << " (" << b_size << " stems), " << get_rss_mb() << " MB RSS" << endl;
            }
            // bucket_postings freed here
        }
        sf.close();
    ...
    cout << "\n=== BUILD COMPLETE (v9.0 — two-pass disk-backed) ===" << endl;
