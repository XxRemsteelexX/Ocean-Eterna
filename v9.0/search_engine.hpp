...
// Loads one segment at a time (mmap), searches, keeps best results,
// then munmaps before loading next. Peak RAM = 1 segment (~1.5GB at 2.5B).
// ============================================================================
...
inline RemoraResult search_sequential_segments(
    const std::vector<std::string>& segment_dirs,
    const std::string& query, int topk,
    double early_stop_score = 0.0)
{
    RemoraResult result;
    result.total_segments = (int)segment_dirs.size();
    ...
    if (query_stems.empty()) return result;

    std::vector<Hit> best_hits;
    double best_top_score = 0.0;

    for (int s = 0; s < (int)segment_dirs.size(); s++) {
        auto seg_start = std::chrono::high_resolution_clock::now();

        // load this segment's mmap index
        MmapIndex midx;
        if (!midx.load(segment_dirs[s])) {
            std::cerr << "[remora] Failed to load segment " << s
                      << " from " << segment_dirs[s] << std::endl;
            continue;
        }

        // create temporary SegmentIndex for search
        SegmentIndex seg;
        seg.id = s;
        seg.dir = segment_dirs[s];
        seg.doc_count = midx.doc_count;
        seg.avg_dl = midx.avg_dl;
        seg.doc_offset = (uint64_t)s * 10000000;  // 10M offset per segment to avoid collisions
        seg.mmap_idx = std::move(midx);

        // BM25 search this segment
        auto seg_hits = search_segment_bm25(seg, query_stems, topk);

        // phase 2: coverage rescoring (load keyword_dict from manifest.bin)
        std::string seg_mbin = segment_dirs[s] + "/manifest.bin";
        if (std::filesystem::exists(seg_mbin)) {
            BinaryCorpus bc = load_binary_manifest_mmap(seg_mbin, nullptr, true);
            seg.keyword_dict = std::move(bc.keyword_dict);
        }
        for (auto& hit : seg_hits) {
            rescore_segment_hit(hit, seg, query_stems, raw_terms);
        }

        // merge into best_hits
        for (auto& h : seg_hits) {
        ...
        std::sort(best_hits.begin(), best_hits.end(),
            [](const Hit& a, const Hit& b) { return a.score > b.score; });
        if ((int)best_hits.size() > topk * 2) {
            best_hits.resize(topk * 2);
        }

        if (!best_hits.empty() && best_hits[0].score > best_top_score) {
            best_top_score = best_hits[0].score;
        }

        auto seg_elapsed = std::chrono::duration<double, std::milli>(
            std::chrono::high_resolution_clock::now() - seg_start).count();
        result.total_search_ms += seg_elapsed;
        result.segments_searched++;

        // release mmap (free RAM)
        seg.mmap_idx.close();

        std::cout << "[remora] Seg " << s << ": " << seg_hits.size()
                  << " hits, best=" << std::fixed << std::setprecision(1)
                  << (seg_hits.empty() ? 0.0 : seg_hits[0].score)
                  << ", " << std::setprecision(0) << seg_elapsed << "ms" << std::endl;

        // early stopping
        if (early_stop_score > 0.0 && best_top_score >= early_stop_score) {
            std::cout << "[remora] Early stop: score " << std::setprecision(1)
                      << best_top_score << " >= " << early_stop_score << std::endl;
            break;
        }
    }
    ...
    std::sort(best_hits.begin(), best_hits.end(),
        [](const Hit& a, const Hit& b) { return a.score > b.score; });
    if ((int)best_hits.size() > topk) {
        best_hits.resize(topk);
    }
    result.hits = std::move(best_hits);
    return result;
}
